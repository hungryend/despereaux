...
# Vendored into despereaux from the user's pdf-to-markdown project (pdf2md.py).
# Requires PyMuPDF (fitz) — AGPL-3.0 (or commercial from Artifex) — installed only
# via despereaux's optional `pdf` extra. fitz is lazy-imported (see _require_fitz)
# so this module stays importable without it. Kept close to upstream for re-sync.
...
# PyMuPDF (fitz) is lazy-imported so despereaux can import this module without the
# optional `pdf` extra installed. _require_fitz() loads it on first real use.
fitz = None
