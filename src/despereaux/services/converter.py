...
async def convert_to_epub(src: Path, out: Path) -> Path | None:
    ...
    and is non-empty, returns it without re-converting."""
    ...
    async with _CONVERSION_LOCK:
        ...
        try:
            ...
            try:
                ...
                proc.kill()
                await proc.wait()
                log.warning("ebook-convert timed out after %ds on %s", _TIMEOUT_SECONDS, src.name)
                if tmp.exists():
