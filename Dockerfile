...
# --extra pdf pulls PyMuPDF (AGPL-3.0) for the PDF -> Markdown -> EPUB pipeline
# (vendored services/pdf2md.py). Drop it to build an AGPL-free image (PDF
# conversion then unavailable; MOBI/AZW still convert via Calibre).
RUN uv sync --no-dev --extra formats --extra pdf
